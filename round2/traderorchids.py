...
class Trader:
    ...
    def run(self, state: TradingState): # state must be a TradingState object
        ...
        for product in state.order_depths: # for each product in the order listing
            ...
            orders, conversions = self.get_orders_and_conversions(product)
    ...
    def get_orders_and_conversions(self, state, records, product):
        if product == "ORCHIDS":
            return self.orchid_algorithm(state, records)
    ...
    def orchid_algorithm(self, state: TradingState, records):
        product = "ORCHIDS"
