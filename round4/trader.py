...
class Trader:
    ...
    def get_orders_and_conversions(self, state, records, product):
        ...
        if product == 'COCONUT_COUPON':
            return self.coconut_coupon_algorithm(state, records)
        return [], 0
    ...
    def coconut_coupon_algorithm(self, state, records):
        product1 = "COCONUT_COUPON"
        product2 = "COCONUT"
        ...
        best_ask_coupon = 0
        best_ask_amount_coupon = 0
        best_bid_coupon = 0
        best_bid_amount_coupon = 0
        best_ask_coconut = 0
        best_ask_amount_coconut = 0
        best_bid_coupon = 0
        best_bid_amount_coupon = 0

        if len(order_depth1.sell_orders) != 0:
            best_ask_coupon, best_ask_amount_coupon = list(order_depth1.sell_orders.items())[0]
        if len(order_depth1.buy_orders) != 0:
            best_bid_coupon, best_bid_amount_coupon = list(order_depth1.buy_orders.items())[0]
        mid_price_coupon = self.get_midprice(order_depth1) # change this to fair value price?

        if len(order_depth2.sell_orders) != 0:
            best_ask_coconut, best_ask_amount_coconut = list(order_depth2.sell_orders.items())[0]
        if len(order_depth2.buy_orders) != 0:
            best_bid_coconut, best_bid_amount_coconut = list(order_depth2.buy_orders.items())[0]
        mid_price_coconut = self.get_midprice(order_depth2) # change this to fair value price?
