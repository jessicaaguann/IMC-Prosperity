...
class Trader:
    ...
    def run(self, state: TradingState): # state must be a TradingState object
        ...
        if len(state.traderData) == 0:
            ...
            records = {"AMETHYSTS": [], "STARFRUIT": [], "SUM_SPREAD": []}
        else:
    ...
    def get_orders_and_conversions(self, state, records, product):
        ...
        if product == "COCONUT_COUPON":
            return self.coconut_algorithm(state, records)
        return [], 0
    ...
    def get_amount_to_sell(self, order_depth: OrderDepth, position, product: str) -> int:
        ...
        best_bid, best_bid_amount = 12340344, 0
    ...
    def orchids_algorithm(self, state: TradingState, records):
        ...
        # if the conversion buy price is slightly greater than what you can sell it at, buy now and try selling a bit higher
            ...
            conversions = -sell_amount
        elif south_buy_price - 1.5 < best_bid:
            sell_amount = self.get_amount_to_sell(order_depth, state.position, product)
            print(f"SELL LOCAL {sell_amount}x{best_bid}")
            orders.append(Order(product, best_bid + 2, sell_amount))
            conversions = -sell_amount
    ...
    def gift_basket_algorithm(self, state, records):
        ...
        if len(records["SUM_SPREAD"]) != 0:
            a = records["SUM_SPREAD"][0] / (state.timestamp / 100) - 35
            b = records["SUM_SPREAD"][0] / (state.timestamp / 100) + 35
        ...
        if len(records["SUM_SPREAD"]) == 0:
            records["SUM_SPREAD"].append(dif)
        else:
            records["SUM_SPREAD"][0] += dif
        return orders, conversions
    
    def black_scholes_call(self, cur_price, strike, expdate, riskfree, sigma):
        d1 = (log(cur_price / strike) + (riskfree + 0.5 * sigma ** 2) * expdate) / (sigma * sqrt(expdate))
        d2 = d1 - sigma * sqrt(expdate)

        call_price = cur_price * self.norm_cdf(d1) - strike * exp(-riskfree * expdate) * self.norm_cdf(d2)
    ...
    def coconut_algorithm(self, state, record):
        conversions = 0
        productnut = "COCONUT"
        productcou = 'COCONUT_COUPON'
        orders: List[Order] = [] 
        od_nut : OrderDepth = state.order_depths[productnut]
        od_cou : OrderDepth = state.order_depths[productcou]

        mid_price_nut = self.get_midprice(od_nut)
        mid_price_cou = self.get_midprice(od_cou)

        buy_amount = 0
        sell_amount = 0

        best_ask = 1232312
        if len(list(od_cou.sell_orders.items())) != 0:
            best_ask, best_ask_amount = list(od_cou.sell_orders.items())[0]

        best_bid = 0
        if len(list(od_cou.buy_orders.items())) != 0:
            best_bid, best_bid_amount = list(od_cou.buy_orders.items())[0]
        ...
        call_price = self.black_scholes_call(mid_price_nut, strike, (exp_period/1000), risk_free_rate, sigma)
        
        print("BS call price", call_price)
        print("Coupon mid price", mid_price_cou)

        # market take
        margin = 5
        if best_ask + margin < call_price: # buy
            buy_amount = self.get_amount_to_buy(od_cou, state.position, productcou)
            print("BUY", str(buy_amount) + "x", best_ask)
            orders.append(Order(productcou, best_ask, buy_amount))

        if best_bid > call_price + margin: # sell
            sell_amount = self.get_amount_to_sell(od_cou, state.position, productcou)
            print("SELL", str(sell_amount) + "x", best_bid)
            orders.append(Order(productcou, best_bid, sell_amount))

        position = self.get_position(productcou, state)
        buy_limit = 600 - position - buy_amount
        sell_limit = -600 - position + sell_amount
        
        
        # market make
        buy_amount = min(buy_limit, 12)
        sell_amount = max(sell_limit, -12)
        ...
        if buy_amount + position > buy_limit:
            buy_amount = 0
        if position + sell_amount < sell_limit:
            sell_amount = 0

        buy_price = int(round(call_price)) - 5
        sell_price = int(round(call_price)) + 5
        
        print("BUY", str(buy_amount) + "x", buy_price)
        orders.append(Order(productcou, buy_price, buy_amount))

        print("SELL", str(sell_amount) + "x", sell_price)
        orders.append(Order(productcou, sell_price, sell_amount))
